...
public class TKDTestHelper {
    ...
    public void white() {
        ...
        for (int i = 0; i < wquestions.size(); i++) {
            ...
            StdDraw.show();
            while (true) {
                if (StdDraw.isMousePressed() && StdDraw.mouseX() >= -340 && StdDraw.mouseX() <= 340
                        && StdDraw.mouseY() >= -300 && StdDraw.mouseY() <= -200) {
                    StdDraw.pause(250);
                    break;
                }
            }
    ...
    /**
     * Plays a special question game for black belt testers. Randonmly selects one
     * question from each belt and uses the collection to play the question game.
     */
    public void blackm() {
